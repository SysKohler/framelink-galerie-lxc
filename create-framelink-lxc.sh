...
TEMPLATE_URL="https://downloads.proxmox.com/images/system/debian-12-standard_12.0-1_amd64.tar.zst"
...
if [ ! -f "$TEMPLATE_STORAGE_PATH/$TEMPLATE_FILE" ]; then
  wget -O "$TEMPLATE_STORAGE_PATH/$TEMPLATE_FILE" "$TEMPLATE_URL"
else
  echo "✅ Template bereits vorhanden."
fi

echo "📦 Erstelle LXC Container $CT_ID..."
pct create $CT_ID \
  $TEMPLATE_STORAGE_PATH/$TEMPLATE_FILE \
  --ostype debian \
  --hostname $HOSTNAME \
  --storage $STORAGE \
